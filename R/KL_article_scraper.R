...
is_article_url <- function(url) {
  ...
  if (str_detect(url, "/\\d{4}/") && nchar(url) > 60) {
    ...
    if (
      !is.na(year_match) &&
        as.numeric(year_match) >= 2015 &&
        as.numeric(year_match) <= 2025
    ) {
      return(TRUE)
    }
  }
          ...
          if (nchar(articles$title[i]) < 20) {
            ...
            for (sel in title_selectors) {
              ...
              if (!is.null(title_node)) {
                ...
                if (
                  !is.na(title_text) &&
                    nchar(title_text) > nchar(articles$title[i])
                ) {
...
main <- function() {
  ...
  # Save QMD to file
  out_path <- here("includes", "kultura-liberalna-articles.qmd")
  writeLines(qmd_output, out_path)
  cat("QMD output saved to", out_path, "\n")
