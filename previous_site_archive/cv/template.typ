...
// Import your CV data
#let cv-data = yaml("complete_cv.yml")
    ...
    if section.at("show", default: true) == true {
      if section.key == "personal" {
        ...
        let temp-data = (
          ...
          (key): section.entries,  // Add this section's entries directly
        )

        // Add layout configuration if present
        if "primary-element" in section {
          temp-data.insert("primary-element", section.primary-element)
        }
        if "secondary-element" in section {
          temp-data.insert("secondary-element", section.secondary-element)
        }
        if "tertiary-element" in section {
          temp-data.insert("tertiary-element", section.tertiary-element)
        }
